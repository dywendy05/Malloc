 ...
 * mm.c
 *
 Segregated(10 segs in total), first fit, double linked, LIFO;
 Seg cuts: 2^4 - 2^5, 2^5 - 2^6, 2^6 - 2^7, 2^7 - 2^8, 2^8 - 2^9, 2^9 - 2^10,
           2^10 - 2^12, 2^12 - 2^14, 2^14 - 2^16, 2^16 - 2^32; 
...
#define SEG_NUM 10
#define PAD 0 
#define LAUNCH_SIZE (((SEG_NUM) + 2) * WSIZE + (PAD))
#define PROLOGUE (void *)((char *)(mem_heap_lo()) + (PAD))
#define FIRST_BLK ((void *)(char *)(PROLOGUE) + ((SEG_NUM) + 2) * WSIZE)
...
#define LIST_HEAD(i) ((void *)((char *)(PROLOGUE) + (((i) + 1) * WSIZE))) 
...
int mm_init(void) {
    ...
    for(int i = 0; i < SEG_NUM; ++i) {
        ptr = (void *)((char *)ptr + WSIZE);
        PUT_PTR(ptr, NULL);
    }
    ptr = (void *)((char *)ptr + DSIZE);
...
/* Insert policy: insert into the front*/
